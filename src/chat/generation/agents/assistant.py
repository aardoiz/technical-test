...
class Assistant:
    ...
    def construct_history(self, user_query: str, conversation: Conversation, docs: str) -> list[dict[str, str]]:
        history = []
    ...
    def answer_query(self, query: str, conversation: Conversation, documents: list[Document]) -> TextResponse:
        i_time = time()

        context = self.build_context(documents)
        history = self.construct_history(query, conversation, context)

        llm_response: ChatCompletion = self.llm_client.generate(history, temperature=0.0, max_tokens=2000)
